...
class HrCnpsMonthly(models.Model):
    ...
    def compute(self):
        ...
        if all_slips:
                            ...
                            if employee:
                                ...
                                if tcnps.type != 'm':
                                    ...
                                    if employee.type == tcnps.type:
                                        if dt['base_jour']:
                                            if dt['base_jour'] > tcnps.amount_min and dt[
                                                'base_jour'] <= tcnps.amount_max:
                                                found = True
                                else:
                                    if employee.type == tcnps.type:
                                        if dt['brut_imposable'] > tcnps.amount_min and dt[
                                            'brut_imposable'] <= tcnps.amount_max:
                                            found = True
                            if found:
                                l_vals['salaries_number'] += 1
                                val = {
                                    'employee_id': employee.id,
                                    'type': employee.type if employee.type else '',
                                    'amount_brut': dt['brut_imposable'],
                                    'daily_basis': dt['base_jour'],
                                    'tranche_id': tcnps.id if tcnps else '',
                                    'date_start': self.date_from if self.date_from else '',
            ...
            self.other_line_ids = self.other_line_ids.create(res)
            ...
            if templates:
                for tpl in templates:
                    ...
                    self.cotisation_ids = self.cotisation_ids.create(cotisations)
    ...
    def export_xls(self):
        ...
        return self.env.ref('hr_payroll_custom.hr_cnps__report_xlsx').report_action(self, data=data)

...
class ACSCommissionBill(models.TransientModel):
    ...
    def create_bills(self):
        ...
        acs_bills = self.env['account.move'].search([('id','in',bills)])
        for bill in acs_bills:
            bill.with_context(check_move_validity=False)._recompute_dynamic_lines(recompute_tax_base_amount=True)

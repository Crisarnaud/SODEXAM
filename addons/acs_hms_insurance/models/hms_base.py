        ...
        if self.invoice_id and self.insurance_id and (self.insurance_id.pha_insurance_limit >= self.invoice_id.amount_total or self.insurance_id.pha_insurance_limit==0):
            ...
            if can_be_splited:
                split_context = {
                    ...
                    'active_id': self.invoice_id.id, 
                    'insurance_id': self.insurance_id.id,
                    'insurance_type': self.insurance_id.pha_insurance_type,
                    'insurance_amount': self.insurance_id.pha_insurance_amount,
                }
                wiz_id = self.env['split.invoice.wizard'].with_context(split_context).create({
                    'split_selection': 'invoice',
                    'percentage': self.pha_insurance_percentage,
                    'partner_id': self.insurance_company_id.partner_id.id if self.insurance_company_id.partner_id else self.patient_id.partner_id.id,
                })
                insurance_invoice_id = wiz_id.split_record()
                insurance_invoice_id.write({
                ...
                if insurance_invoice_id and self.insurance_id.pha_create_claim:
                    claim_id = self.env['hms.insurance.claim'].create({
                        'patient_id': self.patient_id.id,
                        'insurance_id': self.insurance_id.id,
                        'claim_for': 'other',
